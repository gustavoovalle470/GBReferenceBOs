 ...
 */
@Repository
public class GBPwdHistoryDAO implements IGabyvalDAO{
    ...
    public void setEntityManager(SessionFactory session) throws GBPersistenceException {
        ...
        if(this.session==null){
            throw new GBPersistenceException("The session manager can't injected in class GBPwdHistoryDAO", null);
        }
    }
    ...
    public GbPwdHistory load(Serializable o_id) throws GBPersistenceException {
        if(o_id == null || session == null){
            throw new GBPersistenceException("The object in GBPwdHistoryDAO can't load, the session or the object id is null. Contact whit system administrator.", null);
        }
        try{
            return (GbPwdHistory) session.getCurrentSession().get(GbPwdHistory.class, o_id);
        }catch(HibernateException e){
            throw new GBPersistenceException("The save operation can't finished, try again or contact with system administrator.", e);
        }
    }
    ...
    public List<Object> getAll() throws GBPersistenceException {
        ...
        return session.getCurrentSession().createCriteria("From GbPwdHistory").list();
    }    
