 ...
 */
@Repository
public class ADJobDAO implements IGabyvalDAO{
    ...
    public void setEntityManager(SessionFactory session) throws GBPersistenceException {
        ...
        if(this.session==null){
            throw new GBPersistenceException("The session manager can't injected in class ADJobDAO", null);
        }
    }
    ...
    public void save(Object o_to_save) throws GBPersistenceException {
        if(o_to_save == null || session == null){
            throw new GBPersistenceException("The object in ADJobDAO can't save, the session or the object is null. Contact whit system administrator.", null);
        }
    ...
    public AdJob load(Serializable o_id) throws GBPersistenceException {
        if(o_id == null || session == null){
            throw new GBPersistenceException("The object in ADJobDAO can't load, the session or the object id is null. Contact whit system administrator.", null);
        }
        try{
            return (AdJob) session.getCurrentSession().get(AdJob.class, o_id);
        }catch(HibernateException e){
            throw new GBPersistenceException("The save operation can't finished, try again or contact with system administrator.", e);
        }
    }
    ...
    public List<Object> getAll() throws GBPersistenceException {
        if(session == null){
            throw new GBPersistenceException("The object in ADJobDAO can't get all items, the session is null. Contact whit system administrator.", null);
        }
        return session.getCurrentSession().createCriteria("From AdJob").list();
    }    
