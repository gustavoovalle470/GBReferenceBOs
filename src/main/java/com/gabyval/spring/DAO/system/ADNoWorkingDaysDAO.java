 ...
 */
@Repository
public class ADNoWorkingDaysDAO implements IGabyvalDAO{
    ...
    public void setEntityManager(SessionFactory session) throws GBPersistenceException {
        ...
        if(this.session==null){
            throw new GBPersistenceException("The session manager can't injected in class ADNoWorkingDaysDAO", null);
        }
    }
    ...
    public void save(Object o_to_save) throws GBPersistenceException {
        if(o_to_save == null || session == null){
            throw new GBPersistenceException("The object in ADNoWorkingDaysDAO can't save, the session or the object is null. Contact whit system administrator.", null);
        }
    ...
    public AdNoWorkingDays load(Serializable o_id) throws GBPersistenceException {
        if(o_id == null || session == null){
            throw new GBPersistenceException("The object in ADNoWorkingDaysDAO can't load, the session or the object id is null. Contact whit system administrator.", null);
        }
        try{
            return (AdNoWorkingDays) session.getCurrentSession().get(AdNoWorkingDays.class, o_id);
        }catch(HibernateException e){
            throw new GBPersistenceException("The save operation can't finished, try again or contact with system administrator.", e);
        }
    }
    ...
    public List<Object> getAll() throws GBPersistenceException {
        ...
        return session.getCurrentSession().createCriteria("From AdNoWorkingDays").list();
    }    
