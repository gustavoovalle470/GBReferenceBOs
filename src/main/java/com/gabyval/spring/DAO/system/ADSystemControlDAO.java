...
public class ADSystemControlDAO implements IGabyvalDAO{
    ...
    public void save(Object o_to_save) throws GBPersistenceException {
        if(o_to_save == null || session == null){
            throw new GBPersistenceException("The object in ADSystemControlDAO can't save, the session or the object is null. Contact whit system administrator.", null);
        }
        try{
    ...
    public AdSystemControl load(Serializable o_id) throws GBPersistenceException {
        if(o_id == null || session == null){
            throw new GBPersistenceException("The object in ADSystemControlDAO can't load, the session or the object id is null. Contact whit system administrator.", null);
        }
        try{
            return (AdSystemControl) session.getCurrentSession().get(AdSystemControl.class, o_id);
        }catch(HibernateException e){
            throw new GBPersistenceException("The save operation can't finished, try again or contact with system administrator.", e);
        }
    }
    ...
    public List<Object> getAll() throws GBPersistenceException {
        ...
        return session.getCurrentSession().getNamedQuery(GBSentencesRBOs.ADSYSTEMCONTROL_FINDALL).list();
    }    
